# Code reused from https://github.com/pytorch/examples/tree/master/imagenet
import argparse
import os
import random
import shutil
...
parser.add_argument('--data', metavar='DIR', default='/home/lveeramacheneni/tiny-imagenet-200',
...
parser.add_argument('--dataset', default='tinyimagenet', type=str,
                    help='Dataset name choices are tinyimagenet, imagenet & CIFAR10')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Data loading code                                                                            Commenting imagenet dataloading code
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print('Epoch elapsed time is: ', end-start, flush=True)
...
def apply_CP_Norm(model, inference_type = False):
    ...
    Args:
        ...
        inference_type (bool, optional): Boolean variable for inference 
                                         or training mode. Defaults to False.

    Returns:
        [Net]: CP norm applied model instance
    """

    def estimate_ranks(model):
        conv_ranks = []
        linear_ranks = []
        for index, (name, layer) in enumerate(model.named_modules()):
            if isinstance(layer, torch.nn.Conv2d):
                weights = list(layer.weight.shape)
                rank = int(np.ceil(np.prod(weights)/(sum(weights)+1)))
                conv_ranks.append(rank)
            elif isinstance(layer, torch.nn.Linear):
                weights = list(layer.weight.shape)
                rank = int(np.ceil(np.prod(weights)/(sum(weights)+1)))
                linear_ranks.append(rank)
        return conv_ranks, linear_ranks
    # Ranks for convolutional layers
    model = model.cpu()
    c = 0
    conv_ranks, linear_ranks = estimate_ranks(model)
    # Apply CPnorm over convolutional layers
    for index, (name, layer) in enumerate(model.named_modules()):
        if isinstance(layer, nn.Conv2d):
            print('Norm on layer: ', name)
            layer = cp_norm(layer, conv_ranks[c], inference=inference_type, init_method='KNORMAL')
            c+=1
    ...
    for index, (name, layer) in enumerate(model.named_modules()):
        if isinstance(layer, nn.Linear):
            layer = cp_norm(layer, linear_ranks[c], inference=inference_type, init_method='KNORMAL')
            c+=1
    model = model.cuda()
    return model
